...
class TestWorkoutHR:
    ...
    def test_build_workout_hr_uses_daily_hr(self):
        client = MagicMock()
        import datetime as dt
        ...
        samples = build_workout_hr(client, self.WORKOUT)

        assert samples == [{"time": 60.0, "hr": 120}]

    def test_hr_for_sync_reuses_cached_hr(self):
        database = MagicMock()
        database.get_cached_hr.return_value = {"hr_samples": [{"time": 5.0, "hr": 105}]}
        client = MagicMock()

        result = hr_for_sync(
            database, client, self.WORKOUT, {"hr_fusion": {"enabled": True}},
        )

        assert result == [{"time": 5.0, "hr": 105}]
        database.get_cached_hr.assert_called_once_with("w1")
        client.get_heart_rates.assert_not_called()

    def test_hr_for_sync_fetches_daily_hr_on_a_cache_miss(self):
        database = MagicMock()
        database.get_cached_hr.return_value = None
        client = MagicMock()
        import datetime as dt
        start_ms = int(dt.datetime.fromisoformat(self.WORKOUT["start_time"]).timestamp() * 1000)
        ...
        assert result == [{"time": 60.0, "hr": 120}]

    def test_hr_for_sync_never_fails_a_sync(self):
        database = MagicMock()
        database.get_cached_hr.side_effect = RuntimeError("db down")

        result = hr_for_sync(
            database, MagicMock(), self.WORKOUT, {"hr_fusion": {"enabled": True}},
        )

        assert result is None

    def test_hr_for_sync_respects_the_fusion_toggle(self):
        database = MagicMock()

        result = hr_for_sync(
            database, MagicMock(), self.WORKOUT, {"hr_fusion": {"enabled": False}},
        )

        assert result is None
        database.get_cached_hr.assert_not_called()
