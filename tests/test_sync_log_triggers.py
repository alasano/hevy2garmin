...
class TestPerRowSyncIsRecorded:
    ...
    @pytest.mark.parametrize("status", ["synced", "processing", "needs_review", "failed"])
    def test_the_page_is_reloaded_to_show_what_happened(self, client, recorded, monkeypatch, status):
        """The Workouts page renders every status itself; the old reply was a table row
        swapped over a card, and said Synced whatever sync_one_workout returned."""
        from types import SimpleNamespace
        ...
        workout = {"id": "w1", "title": "Push", "start_time": "2026-04-01T20:00:00+00:00", "exercises": []}
    ...
    def test_an_exception_records_a_failure_and_still_renders(self, client, recorded, monkeypatch):
        ...
        def _boom(**kw):
            raise RuntimeError("Hevy said <b>no</b>")

        monkeypatch.setattr(hevy_mod, "HevyClient", _boom)
        resp = client.post("/api/sync/w1")
        assert resp.status_code == 200
        assert "Failed: Hevy said &lt;b&gt;no&lt;/b&gt;" in resp.text
