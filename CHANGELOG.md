...
### Fixed
- "Lateral Raise (Dumbbell)" no longer shows in Garmin as "Leaning Dumbbell Lateral Raise". Twenty-three built-in mappings pointed at a qualified variant, or at a different exercise, although Garmin lists the exercise itself: Lateral Raise (Dumbbell), Chin Up (also assisted and weighted), Pull Up (Assisted), Bicep Curl (Dumbbell), Shoulder Press (Dumbbell), Incline Bench Press (Smith Machine), Upright Row (Cable), Crunch (Machine), Overhead Dumbbell Lunge, Dumbbell Snatch, Snatch, Split Jerk, Box Jump, Jump Squat, Straight Leg Deadlift and Inverted Row; and, previously a different exercise, Reverse Lunge (Dumbbell), Clean and Press, Romanian Deadlift (Barbell and Dumbbell, now Romanian Deadlift without the equipment word) and Ab Scissors (now Flutter Kicks). Lateral Raise (Band), Around The World and Clamshell now use their category alone, because their previous targets were a wrong exercise or a name Garmin Connect does not have. Every built-in target is now tested against Garmin Connect's own exercise list. Workouts that are already synced keep the old name until they are unsynced and synced again.
- Merging a workout into the same Garmin activity a second time (after an unsync) no longer overwrites the backup of the sets the watch recorded with the Hevy sets pushed the first time.
- The Mappings page said to use Sub ID 0 for a generic mapping and defaulted to it. 0 is the first exercise of each category; the generic value is 65535, which is now the default and what the page says.
...
- A failed push of the sets into a watch activity now fails that workout's sync so it is retried. It used to fall through to the start-time check, which found the same watch activity, renamed it without sets and recorded it as an upload. The same applies once the circuit breaker has tripped after three failed pushes in one run.
...
- Sync Now reports a workout that was skipped after an error instead of "Everything is already synced", also when other workouts synced or Stop was pressed, and when nothing synced its closing message says what happened (busy, stopped, still processing) instead of always "Everything is already synced". A known hard stop (upload consent, Hevy key, Garmin login) is shown once, with its link, also after other workouts synced, and an expired Garmin login shows the Reconnect link again (the check looked for words the server no longer sends). A rejected upload or one that needs review is no longer reported as still processing on Garmin. Messages that carry a workout title are inserted as text.
- The "On Garmin" number no longer drops to the count of the current run while Sync Now is running.
- The Workouts page's per-row sync reloads the page, which shows what happened to the workout (processing, needs review, failed). It used to swap a table row that always said "Synced" over the workout's card.
